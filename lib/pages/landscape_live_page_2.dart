...
/// 横屏播放页面 - 使用独立控制器 (解决黑屏/闪退问题)
class LandscapeLivePage2 extends StatefulWidget {
  ...
  @override
  State<LandscapeLivePage2> createState() => _LandscapeLivePage2State();
}

class _LandscapeLivePage2State extends State<LandscapeLivePage2> {
  ...
  void _init() {
    ...
    if (_isLive) {
      ...
      Future.delayed(const Duration(milliseconds: 300), () async {
        if (mounted && !_isLive) {
          try {
            await _agoraService.mediaPlayer?.resume();
          } catch (e) {
  ...
  void dispose() {
    ...
    // 不要在此处 Pause，否则会打断 LivePage2 的 Resume
    // if (!_isLive) {
    //   _agoraService.mediaPlayer?.pause().catchError((_) {});
    // }
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              child: Row(
                children: [
                  Image.asset('assets/splash_logo.png', width: 30, height: 30),
